...
Pix recebido (`pix.in.*`), ciclo de payout (`pix.payout.*`), cobranças
...
| `pix.payout.refunded`       | Estorno relacionado ao payout (parcial ou total). Veja exemplos abaixo. |
...
<AccordionGroup>
  ...
  <Accordion title="pix.payout.refunded">
    ...
    **`amountCents`** é o valor original do payout. **`refundAmountCents`** é o valor **deste** estorno individual. Para detectar refund parcial, compare `refundAmountCents < amountCents`. Estornos parciais geram vários webhooks separados — cada um com `eventId` único quando a plataforma o enviar no envelope (referência conceitual: `payout-refunded-{payoutId}-{refundEndToEndId}`).
  ...
  <Accordion title="pix.charge.refunded">
    **`webhook-event-type`**: `pix.charge.refunded`
    ...
    Mesmo padrão dos payouts pagos via refund: **`amountCents`** = valor original da cobrança que foi liquidada; **`refundAmountCents`** = valor deste refund. Para **R$ 100,00** pagos (**`"10000"`** centavos), um refund de **R$ 25,00**:
