...
O corpo do `POST .../webhooks` exige `events` como array de strings (veja o
schema no OpenAPI). Quais nomes de evento são aceitos para **sua** conta pode
variar — confira no painel ou nos webhooks já cadastrados (`GET` no mesmo recurso).
